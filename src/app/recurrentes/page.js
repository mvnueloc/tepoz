...
export default function Home() {
  ...
  const [recurringTitle, setRecurringTitle] = useState(null);
  const [recurringDescription, setRecurringDescription] = useState(null);
  const [name, setName] = useState("Tio Antonio");
  const [letter, setLetter] = useState("A");
  const [monto, setMonto] = useState("100.00");
  const [concepto, setConcepto] = useState("Carne asada.");
  const [fecha, setFecha] = useState("13 de Octubre de 2024");

  // <-- peticion para el modal -->
  useEffect(() => {
    const fetchModal = async () => {
      try {
        const response = await fetch(
          "https://gemini-328383011109.us-central1.run.app/recurringExpenses"
        );
        if (!response.ok) {
        ...
        setRecurringTitle(recurring.Title);
        setRecurringDescription(recurring.description);
        setName(recurring.name);
        setLetter(recurring.letter);
        setMonto(recurring.monto);
        setConcepto(recurring.concepto);
      } catch (error) {
  ...
  const sendAudio = async (audioBlob) => {
    ...
    try {
      // setIsLoading(true);
      const response = await fetch(
        "https://whisper-328383011109.us-central1.run.app/audio-to-text",
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <>
      ...
      <Modal
        ...
        {recurringTitle ? (
          <div>
            <h2 className="text-2xl font-bold mb-4 ">{recurringTitle}</h2>
